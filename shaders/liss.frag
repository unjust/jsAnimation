...
void main() {
  ...
  float v = 0.0;
  for (int i = 0; i < 200; i++) {
    float s = u_time + float(i) * 0.0075; //  + u_speed/100.0;
    // vec2 mpos = 0.8 * vec2(sin(s * u_mouse.x * 5.0), -cos(s * u_mouse.y * 6.0)); // 0.8 is radius?
    vec2 mpos = 0.8 * vec2(sin(s * u_xy_factor.x * 5.0), -cos(s * u_xy_factor.y * 6.0)); // 0.8 is radius?
