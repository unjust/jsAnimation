...
function loadOctahedrons() {
  for (let i = 0; i < NUM; i++) {
    ...
    const oct_wf = new THREE.OctahedronGeometry(10, 0);
    let material_wf;
    material_wf = new THREE.MeshBasicMaterial({
      color: 0xFFFFFF,
      wireframe: true,
      wireframeLinewidth: 20
    });
    ...
    const mesh_wf = new THREE.Mesh(oct_wf, material_wf);
    scene.add(mesh_wf);
    shapes.push(mesh_wf);
    positionShape(mesh_wf);
  }
}

function positionShape(shape) {
  shape.position.set(0, 0, 170);
};
